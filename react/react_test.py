...
class ReactTest(unittest.TestCase):
    ...
    def test_callbacks_not_called_so_long_as_output_not_changed(self):
        ...
        plus_one = ComputeCell([input_], lambda inputs: inputs[0] + 1)
        minus_one = ComputeCell([input_], lambda inputs: inputs[0] - 1)
        minus_one_more = ComputeCell([minus_one], lambda inputs: inputs[0] - 1)
        plus_one_more = ComputeCell([plus_one], lambda inputs: inputs[0] + 1)
        always_four = ComputeCell(
            [plus_one_more, minus_one_more],
            lambda inputs: inputs[0] - inputs[1]
        )
        ...
        always_four.add_callback(callback1)
        input_.value = 2
        input_.value = 3
        input_.value = 4
        input_.value = 5
